...
class DatabaseHelper {
  static const String BASE_URL = 'http://localhost/rencana_recehsql/';
      ...
      return data.map((item) {
        ...
        return FinancialTransaction.fromMap({
          'id': int.tryParse(item['id'].toString()) ??
              0, // Mengonversi ke int, default 0 jika gagal
          'type': item['type'] as String, // Pastikan 'type' adalah String
          'amount': item['amount'] is String
              ? double.tryParse(item['amount']) ?? 0.0
              : item['amount'], // Pastikan 'amount' adalah double
          'description': item['description']
              as String, // Pastikan 'description' adalah String
